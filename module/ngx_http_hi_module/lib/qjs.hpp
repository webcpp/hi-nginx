...
private:
    JSRuntime* vm;
    JSContext* ctx;
    size_t ctx_called_count, memory_limit, ctx_called_limit;
...
public:
    qjs(size_t memory_limit = 1048576 * 1024, size_t ctx_called_limit = 10240)
        : vm(0)
        , ctx(0)
        , ctx_called_count(0)
        , memory_limit(memory_limit)
        , ctx_called_limit(ctx_called_limit)
        , jsfile_mmap()
    {
